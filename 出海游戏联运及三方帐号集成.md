...
解决方案是：上架GooglePlay的包名和上架AG的APK包名设置为不一致，上架GooglePlay的包，包名没有限制，可以不带.huawei后缀，但是上架AG的包，根据联运规则有两个选择: 选择1：根据联运规则，需要使用.huawei/.HUAWEI后缀，但是如果该应用集成facebook账号，为保证facebook账号的可用性，应用需要[上架APKPURE]，上架方法如下 https://apkpure.com/developer.html
